...
public class CombatUnit : Unit
{
    ...
    protected RaycastHit2D CheckRaycast(Vector2 direction)
    {
        ...
        if (hitInfo.collider != null)
        {
            if (this.tag == "Player1")
            {
                if (hitInfo.collider.CompareTag("Player2") || hitInfo.collider.CompareTag("Tower"))
                {
            ...
            if (this.tag == "Player2")
            {
                if (hitInfo.collider.CompareTag("Player1") || hitInfo.collider.CompareTag("Tower"))
                {
                    _state = UnitState.interact;
                }
                else
